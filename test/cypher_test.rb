...
class CypherTest < Minitest::Test
  ...
  def test_generates_a_rotation_cypher
    key = "12345"
  ...
  def test_it_generates_key_cycle
    actual = @cypher.generate_key_cycle("12345")

    assert_instance_of Enumerator, actual
    assert_equal ["1", "2"], actual.next
    assert_equal [["1", "2"], ["2", "3"], ["3", "4"], ["4", "5"]], actual.to_a
  end
